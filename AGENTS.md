...
- Model evaluations compare the candidate commit with the last published tag in independent clean contexts. Only train failures guide description edits; validation results select the final iteration. A behavior case that expects a file change or source-specific finding must list every required fixture; a case without listed files must be verifiable against the neutral fixture alone. Start every `(case or query, version, attempt)` from a fresh target populated from the candidate checkout, verify its manifest and SHA-256 hashes before execution, and discard it afterward.
