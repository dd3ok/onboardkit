...
- Generated root `AGENTS.md` has no minimum length or fixed sections, normally stays at or below 200 words, and contains only supported commands, recurring conventions, scoped safety rules, or proven routing. Evidence summaries, descriptive paths, workflow-only setup steps, missing commands, uncertainty, and conflicts belong only in the report.
