...
Authority: canonical vocabulary and role contracts for onboardkit core workflows.
...
onboardkit uses these terms so agents do not confuse "a command ran" with "work is done". This document defines words only. It does not add orchestration, subagent routing, or a task ledger.
...
criterion: a user, project, or harness requirement that can be checked. A criterion may be command-backed, artifact-backed, manual, or unsupported by the current helper.
...
adapter: optional host-specific or tool-specific glue that maps onboardkit contracts to another environment without changing the core contract.
