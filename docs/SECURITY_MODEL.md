...
onboardkit is a repo preparation skill and evidence helper. It is not an OS sandbox. It must cooperate with the host agent's sandbox, approval policy, and network controls.
...
The bundled helper's `verify` command evaluates command criteria before execution.
...
- Built-in defaults mark push, release, network fetch, infrastructure, and remote shell commands as prompt-required.
...
The bundled helper's `doctor --security` emits stable findings with IDs:

- `ONBOARDKIT-SEC-001`: AGENTS.md security guardrails.
- `ONBOARDKIT-SEC-002`: safe `.codex/config.example.toml`.
- `ONBOARDKIT-SEC-003`: no unsafe active `.codex/config.toml`.
- `ONBOARDKIT-SEC-004`: runtime outputs ignored by git.
- `ONBOARDKIT-SEC-005`: command policy fail-closed defaults.
- `ONBOARDKIT-SEC-006`: evidence secret redaction patterns.
...
- The helper cannot prevent a host agent from running commands outside the helper.
