...
5. **Placement:** Keep policy detail in its fitting canonical file or narrowest existing audience/scope destination. Create root `AGENTS.md` only for repo-wide guidance lacking an active root or fitting destination. The only exception is a short root pointer to a fitting canonical safety policy that agents must see before they can reasonably discover it; the pointer may name a narrower scope but never copies policy detail.
